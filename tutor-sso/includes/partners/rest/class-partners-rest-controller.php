<?php
/**
 * REST API for creating / updating "partner" (organization) posts.
 *
 * Exposes a single upsert endpoint:
 *
 *   POST /wp-json/rwaq/v1/partners
 *
 ...
 *
 * Create vs. update is decided by `partner_id`: if a partner with that id
 * already exists it is updated, otherwise a new one is created, so one LMS
 * organization maps to one post.
 *
 ...
 *   - partner_id  → ACF field, upsert key  (required, immutable)
 *   - image       → sideloaded + set as the post's featured image
 *
 * Mirrors the instructors controller field-for-field, minus `course_count`:
 * a partner's course and program totals are read live from the organizations API
 * by the detail page, so storing a copy in WordPress would only go stale.
 *
 * @package tutor-sso
 */
...
/**
 * Upsert controller for the `partner` custom post type.
 */
class Partners_REST_Controller extends \WP_REST_Controller {

	/** Remembers the source URL of the featured image to avoid re-downloading. */
	const FEATURED_IMAGE_META = '_partner_featured_image_source';
	...
	public function __construct() {
		...
		/**
		 * Filter the REST base for the partners endpoint.
		 *
		 * Defaults to 'partners', matching the WordPress post type. Set it to
		 * 'organizations' if the syncing client is written against the LMS's own
		 * naming instead.
		 *
		 * @param string $rest_base Default 'partners'.
		 */
		$this->rest_base = (string) apply_filters( 'tutor_sso_partner_rest_base', 'partners' );
	}

	/**
	 * The custom post type this endpoint manages.
	 *
	 * Shares the `tutor_sso_partner_post_type` filter with the archive and detail
	 * views, so every side always agrees on the post type.
	 *
	 * @return string
	 */
	public static function post_type() {
		if ( function_exists( '\TutorSSO\partners_archive_post_type' ) ) {
			return \TutorSSO\partners_archive_post_type();
		}
	 ...
	 * separately, so none of those appear here.
	 *
	 * @return array<string,string> request_field => acf_field_name
	 */
	public static function acf_field_map() {
		$lms_id_field = defined( '\TutorSSO\PARTNER_LMS_ID_FIELD' )
			? \TutorSSO\PARTNER_LMS_ID_FIELD
			: 'partner_id';

		/**
		 * Filter the request-field → ACF-field-name map for partners.
		 *
		 * @param array<string,string> $map Default field map.
		 */
		return apply_filters(
			'tutor_sso_partner_acf_fields',
			array(
				'partner_id' => $lms_id_field,
			)
		);
	}

	/**
	 * Fields that must be present when creating a new partner.
	 *
	 ...
	 *
	 * Only `partner_id` is schema-required (it locates the partner on every
	 * request). `title` and `slug` are required only when *creating*, which is
	 * enforced in the create branch so partial updates are not blocked — the same
	 * pattern used for instructors, courses and programs.
	 *
	 * @return array
	 */
	public function get_endpoint_args() {
		return array(
			'title'       => array(
				'description'       => __( 'Organization name — used as the post title. Required on create.', 'tutor-sso' ),
				'type'              => 'string',
				'required'          => false,
				'sanitize_callback' => 'sanitize_text_field',
			),
			'description' => array(
				'description'       => __( 'Organization description → post content (HTML allowed).', 'tutor-sso' ),
				'type'              => 'string',
				'required'          => false,
				'sanitize_callback' => 'wp_kses_post',
			),
			'slug'        => array(
				'description'       => __( 'Organization slug (post_name). Required on create; on update it is only changed when supplied.', 'tutor-sso' ),
				'type'              => 'string',
				'required'          => false,
			...
			'partner_id'  => array(
				'description'       => __( 'Numeric organization id on the LMS. Unique upsert key; immutable on update.', 'tutor-sso' ),
				'type'              => 'string',
				'required'          => true,
				'sanitize_callback' => 'sanitize_text_field',
			...
			'image'       => array(
				'description'       => __( 'URL of a logo/image to download and set as the featured image.', 'tutor-sso' ),
				'type'              => 'string',
				'format'            => 'uri',
				'required'          => false,
	...
	public function upsert_permissions_check( $request ) {
		...
		/**
		 * Filter the capability required to create/update partners via the API.
		 *
		 * @param string           $capability Default 'edit_posts'.
		 * @param \WP_REST_Request $request    The current request.
		 */
		$capability = apply_filters( 'tutor_sso_partner_api_capability', 'edit_posts', $request );

		if ( ! current_user_can( $capability ) ) {
			return new \WP_Error(
				'tutor_sso_rest_forbidden',
				__( 'You do not have permission to manage partners.', 'tutor-sso' ),
				array( 'status' => 403 )
			);
		}
	...
	/**
	 * Create or update a partner from the request body.
	 *
	 * @param \WP_REST_Request $request Request object.
	...
	public function upsert_item( $request ) {
		...
		$lms_id      = $request->get_param( 'partner_id' );
		$slug        = $request->get_param( 'slug' );
		$status      = $request->get_param( 'status' );
		$existing_id = $this->find_partner_by_lms_id( $lms_id );
		...
		if ( $existing_id ) {
			...
			// Reject a slug already taken by another partner before creating.
			...
			if ( ! isset( $postarr['post_status'] ) ) {
				/**
				 * Filter the post status new partners are created with.
				 *
				 * @param string $status Default 'publish'.
				 */
				$postarr['post_status'] = apply_filters( 'tutor_sso_partner_default_status', 'publish' );
			}
		...
		// Surface image trouble without failing the whole request — the partner
	...
	/**
	 * Find an existing partner by its LMS organization id.
	 *
	 * @param string $lms_id LMS organization id.
	 * @return int Post ID, or 0 when none matches.
	 */
	protected function find_partner_by_lms_id( $lms_id ) {
		$ids = get_posts(
			array(
				'post_type'        => self::post_type(),
				'post_status'      => 'any',
				...
				'meta_query'       => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
					array(
						'key'   => self::acf_field_map()['partner_id'],
						'value' => $lms_id,
					),
				),
			)
	...
	/**
	 * Find an existing partner by its slug (post_name).
	 *
	 * @param string $slug Partner slug.
	 * @return int Post ID, or 0 when none matches.
	 */
	protected function find_partner_by_slug( $slug ) {
		$ids = get_posts(
			array(
				'post_type'        => self::post_type(),
				'post_status'      => 'any',
	...
	/**
	 * Check whether a slug is already taken by another partner.
	 *
	 * @param string $slug      Slug to check.
	 * @param int    $ignore_id Partner to exclude from the lookup — the one being
	 *                          updated, so re-sending its own slug is not a clash.
	 * @return \WP_Error|null Error when the slug belongs to a different partner.
	 */
	protected function slug_conflict_error( $slug, $ignore_id = 0 ) {
		$owner_id = $this->find_partner_by_slug( $slug );
		...
		return new \WP_Error(
			...
			/* translators: %s: slug */
			sprintf( __( 'A partner with the slug "%s" already exists.', 'tutor-sso' ), $slug ),
			array( 'status' => 409 )
		);
	}
	...
	protected function save_fields( $post_id, $request, $is_update = false ) {
		...
		$immutable = array( 'partner_id' );

		foreach ( self::acf_field_map() as $param => $field_name ) {
			// Only write fields that were actually sent, so a partial update does
			// not wipe existing values.
	 ...
	 * (by any partner, instructor, course or program) instead of duplicating it.
	 *
	...
	/**
	 * Public schema for the partner resource (returned by OPTIONS requests).
	 *
	 * @return array
	 */
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				'id'         => array(
					'description' => __( 'Partner post ID.', 'tutor-sso' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'action'     => array(
					'description' => __( 'Whether the partner was created or updated.', 'tutor-sso' ),
					'type'        => 'string',
					'enum'        => array( 'created', 'updated' ),
					'readonly'    => true,
				),
				'partner_id' => array(
					'description' => __( 'LMS organization id.', 'tutor-sso' ),
					'type'        => 'string',
				),
				'link'       => array(
					'description' => __( 'Partner permalink.', 'tutor-sso' ),
					'type'        => 'string',
					'format'      => 'uri',
					'readonly'    => true,
				),
